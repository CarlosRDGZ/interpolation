const Interpolation = function () {
    ...
    function linear(x, xs, fxs) {
        return new Promise((resolve, reject) => {
            if (xs.length !== 2 && fxs.length !== 2) reject("Invalid data");

            for (let i = fxs.length - 1; i >= 0; i--) {
                fxs[i] = Number(fxs[i]);
                if (isNaN(fxs[i])) reject("Non numeric value resived");
            }

            for (let i = xs.length - 1; i >= 0; i--) {
                xs[i] = Number(xs[i]);
                if (isNaN(xs[i])) reject("Non numeric value resived");
            }
    ...
    function quadratic(x, xs, fxs) {
        return new Promise((resolve, reject) => {
            ...
            resolve(
                fxs[0] + (b1(xs, fxs) * (x - xs[0])) + (b2(xs, fxs) * (x - xs[0]) * (x - xs[1]))
            );
        })
    }
